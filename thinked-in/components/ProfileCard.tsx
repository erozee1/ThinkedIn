...
export default function ProfileCard({
  ...
  if (compact) {
    // Horizontal compact row — used in landing demo
    ...
    const inner = (
      <>
        <Avatar
          ...
          name={person.name}
          size={36}
          className="rounded-full object-cover ring-2 ring-black/5 shrink-0"
        />
    ...
    return (
      <a href={person.linkedinUrl} target="_blank" rel="noopener noreferrer" className={`group transition-colors hover:bg-black/[0.03] ${base}`}>
        {inner}
      </a>
    );
  }

  // LinkedIn-style vertical card — used in chat matches grid
  const card = (
    <div className="overflow-hidden rounded-xl border border-zinc-200 bg-white shadow-sm transition-shadow group-hover:shadow-md">
      ...
      {/* Body — relative+z-10 so the avatar stacks above the positioned banner div */}
